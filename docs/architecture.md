...
The approved [Source revisions and change detection](research/source-revisions-and-change-detection.md) design specifies manual one-source refresh, current/history revision selection, immutable historical citations, and failure-safe publication. This follow-up behavior is not implemented yet; `task-08cdcec9` owns the implementation, while ordinary ingestion continues to stage changed bytes as described above.
