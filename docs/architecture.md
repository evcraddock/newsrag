...
`newsrag refresh <source-id>` implements the approved [Source revisions and change detection](research/source-revisions-and-change-detection.md) policy. It enqueues one background full-byte source acquisition, reuses the common adapter/indexing pipeline, and atomically publishes a complete revision and current pointer only after success. Outcomes are `unchanged`, `revision_created`, `revision_reactivated`, or `duplicate_ignored`; failed jobs leave the current revision intact. Ordinary ingestion continues to stage changed bytes as described above.

Schema version 6 adds source revision membership, explicit current pointers, monotonic publication generations, and separate user metadata origin. Initial ingestion publishes revision 1. Migration preserves old document/source-unit IDs and historical metadata, conservatively marks inherited legacy metadata, and rejects multiple ambiguous unversioned documents rather than choosing a winner. Historical revisions and their indexes remain published; only their current designation changes.

Search, packets, and corpus discovery default to current published documents and accept `--include-history`. Retrieval fixes an eligible-document snapshot across keyword/vector candidate selection, expansion, and ranking; packet provenance resolves the exact selected revision even if refresh completes concurrently. Inventory retains all published documents, and `documents versions <document-id>` exposes ordered revision history and its current marker.
...
Remote acquisition resolves and pins a validated public address for each request, revalidates every redirect destination, follows at most five redirects, and applies a 30-second request timeout. URL credentials, localhost, non-public addresses, unsafe redirects, ambient proxy or cookie state, and unsupported content encodings are rejected. HTTP bodies are streamed with separate 250 MiB compressed and decompressed limits, reduced to 10 MiB when a supported HTML, plain-text, or Markdown hint, extension, or response media type identifies that format. Known DOCX input uses a 25 MiB acquisition cap before ZIP expansion is separately validated. Only the submitted resource and required HTTP redirects are fetched; linked and embedded resources are not retrieved.

Local acquisition has a 250 MiB limit, reduced to 10 MiB for recognized HTML, plain-text, or Markdown input, and accepts only regular files. An explicitly submitted symlink is allowed only when its resolved regular-file target remains stable. Directory scans skip symlinks. Device/inode, size, modification time, resolved target, and bytes read are checked around the read so missing, special, changed, or oversized inputs fail without preserving a partial artifact.
...
The Markdown adapter shares text decoding and bounds and uses a versioned CommonMark block parser with tables enabled. It emits `markdown_block` units with raw source slices, heading paths, list/quote ancestry, and original inclusive line ranges, retaining blank gaps and code delimiters. It neither renders Markdown/HTML nor fetches links or embedded resources. Saved charset and parser settings participate in reproducible processing; refresh retains accepted Markdown evidence for generic sources. See [Markdown sources](markdown.md).

The DOCX adapter validates a bounded transitional OOXML ZIP package without extracting files or invoking an Office runtime. Required parts, content types, XML trees, relationship targets, CRCs, and expansion limits are checked. Only external hyperlink targets are ignored with visible text retained; other external relationships and active/embedded content fail. Body paragraphs, headings, lists, tables, and footnotes become `docx_block` units with native paragraph/table/footnote locations and heading paths. Embedded images are neither decoded nor OCRed. See [DOCX sources](docx.md) for package limits, supported structure, and provenance semantics.

## Chunking and citations

PDF uses page-first chunking. Each page is stored as canonical extracted text. Short pages may produce one chunk; long pages are split into overlapping passages while preserving page start/end. HTML uses block-first chunking, keeping each source unit tied to its stable block ordinal and heading path. Plain text uses the same non-page chunker, retaining original line numbers through blank lines and same-line splits. Markdown uses the same non-page chunker with heading paths and inclusive line ranges; a split block retains its full original block location. DOCX uses the same non-page chunker with heading-aware paragraph/table/footnote locations. Shared chunks and passages retain authoritative source-unit start/end IDs for all formats.
...
Search filters should use user-supplied civic metadata, including body, document type, meeting date ranges, jurisdiction/source, and source URL where useful. Search spans every indexed source type by default; the optional `--source-type` filter on `newsrag search` and `newsrag documents list` narrows results to `pdf`, `html`, `text`, `markdown`, or `docx` while composing with the existing filters.

Document inventory derives source type from the published artifact and reports typed extents rather than treating every document as paginated: PDFs show page counts, HTML and DOCX documents show canonical block counts, and plain text and Markdown show original line counts including blanks. Search citations likewise resolve persisted typed source-unit locations, preserving PDF page, HTML heading/block, plain-text line, Markdown heading/line-range, and DOCX heading/paragraph/table/footnote citations.
...
Refresh publication marks its job done in the same SQLite transaction as the document/revision/current-pointer update. Failed jobs retain a verified artifact checkpoint and original publication generation; retry resumes saved bytes after acquisition, while a fresh refresh reacquires the source. Generation conflicts require a new refresh. A process-held corpus worker lock serializes refresh execution/recovery and prevents takeover of a live or cancelling worker; ordinary ingestion workers remain concurrent. Interrupted refreshes without committed completion become failed/retryable. Stop old daemon versions before upgrading a corpus, then restart all workers on the new version.
