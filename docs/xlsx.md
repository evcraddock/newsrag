...
Every package part, including hidden or unindexed content, is validated using shared hardened OPC ZIP/XML machinery with an XLSX-specific allowlist. Accepted workbook, worksheet, shared-string, style, and native-table structures also use closed child/attribute rules, including nested inert display metadata; unknown descendants, malformed leaves, and invalid metadata value types are rejected rather than ignored. DOCX retains its independent policy and limits. Validation checks aliases/traversal, symlinks, encryption/compression, actual expansion and CRCs, content associations, roots, relationship ownership/references/reachability, and strict XML without DTD/entities/network access or recovery. No ZIP member is extracted to a filesystem path.
